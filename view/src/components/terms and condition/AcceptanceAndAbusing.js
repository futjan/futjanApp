import React from "react";
import { Typography } from "@mui/material";
export default function AcceptanceAndAbusing() {
  return (
    <>
      <Typography variant="h4">Acceptance/Abusing FutJan</Typography>
      <br />
      <Typography variant="p" display="block" textAlign="justify">
        FutJan and its community work together to keep FutJan working properly
        and community safe. Please report problems, offensive content and policy
        breaches to us using the reporting system.
        <br />
        <br />
        FutJan provide a collection of online resources which include classified
        advertisements (collectively, the "Service") on the website
        www.FutJan.com and corresponding and the mobile application of the
        platform (collectively, the "Platform").
        <br />
        <br />
        The Platform and the Service are provided to you subject to these FutJan
        Terms of Use (these "Terms"). For the purpose of the Terms and wherever
        the context so requires, the terms "you", "your" and “users” shall mean
        any person who uses or accesses, whether through manual or automated
        means, the Platform or the Service in any manner whatsoever including
        persons browsing the Platform and its content, posting comments or any
        content or responding to any advertisements or content on the Platform.
        By using the Service, you agree to comply with these Terms.
        Additionally, when using a portion of the Service, you agree to conform
        to any applicable posted guidelines for such Service, which may change
        or be updated from time to time at FutJan’s sole discretion. You
        understand and agree that you are solely responsible for reviewing these
        Terms from time to time. Should you object to any term or condition of
        these Terms, any guideline, or any subsequent changes thereto or become
        unhappy with FutJan or the Service in any way, your only choice is to
        immediately discontinue the use of FutJan. These Terms may be updated by
        FutJan at any time at its sole discretion. FutJan may send you notices
        of changes to the Platform or the Terms pursuant to Section xxi (I)
        herein. FutJan may provide a translation of the English version of the
        Terms into other languages. You understand and agree that any
        translation of the Terms into other languages is for your convenience
        only and that the English version governs the terms of your relationship
        with FutJan. Furthermore, if there are any inconsistencies between the
        English version of the Terms and any translation, the English version of
        the Terms shall govern.
      </Typography>
    </>
  );
}
