...
const BusinessTable = (props) => {
  ...
  // useEffect
  useEffect(() => {
    dispatch(getBusinessesPrivate(page, rowsPerPage));
  }, [page, rowsPerPage]);
  ...
  // get state from store
  const surplus = useSelector((state) => state.business);
  // useEffect
  useEffect(() => {
    dispatch(getBusinessesPrivate(page * 1, rowsPerPage));
  }, []);

  return (
    <>
      <h1>Business ({surplus.totalDocs})</h1>
      <div className="table-responsive">
        <Table
          ...
          rows={surplus.privateBusinesses.map((bus) => {
            return {
              _id: bus._id,
              images: bus.images,
              title: bus.title,
              ad_id: bus.ad_id,
              category: bus.businessType,
              active: bus.active,
            };
          })}
          setId={setId}
          getOneProduct={getBusinessById}
          setModal={setModal}
          headCells={[
            {
              id: "image",
            ...
            surplus.surplus._id ? (
              <div style={{ gap: "10px", display: "flex" }}>
                ...
                <Button
                  ...
                  className="warning"
                  onClick={() => {
                    dispatch(deleteBusiness(surplus.surplus._id));
                    setModal(false);
                  }}
                >
                  Delete
                  ...
                  <td className="text-center">
                    <div className="input-group btn-block">
                      ...
                      <Button
                        ...
                        color="error"
                        variant="contained"
                        onClick={() => dispatch(getBusinessById(surplus._id))}
                      >
                        DELETE
                      </Button>
